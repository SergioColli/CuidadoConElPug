...
function loadProducts() {
  ...
  products.forEach(product => {
    ...
    productCard.innerHTML = `
      ...
      <button onclick="addToCart('${product.name}', '${product.gender}', '${product.size}', ${product.price}, '${product.image}')" class="buy-button">Comprar</button>
...
function addToCart(name, gender, size, price, image) {
  let cart = JSON.parse(localStorage.getItem("cart")) || [];
  
  // Verificar si el producto ya está en el carrito
  const existingProduct = cart.find(item => item.name === name && item.size === size && item.gender === gender);

  if (existingProduct) {
    existingProduct.quantity += 1; // Si el producto ya está en el carrito, aumentar la cantidad
  } else {
    // Si el producto no está en el carrito, agregarlo
    cart.push({ name, gender, size, price, image, quantity: 1 });
  }
...
function updateCartCount() {
  ...
  cartIcon.textContent = cartCount; // Actualizar el contador en el ícono del carrito
}

// Función para mostrar subcategorías de productos
...
window.onload = function() {
  loadProducts();
  updateCartCount(); // Para actualizar el contador de carrito al cargar
};
